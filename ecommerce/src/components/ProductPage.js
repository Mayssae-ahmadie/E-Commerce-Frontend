...
const AllProductPage = () => {
  ...
  const catFoodProducts = productData.filter(
    (product) => product.category === "Cat Food"
  );
  const dogFoodProducts = productData.filter(
    (product) => product.category === "Dog Food"
  );
  const accessoryProducts = productData.filter(
        ...
        <div>
          <div className="grid grid-cols-3 gap-4 p-10">
            {catFoodProducts.map((product) => (
              <div key={product._id}>
                <div
                  className="block justify-center items-center border-2 border-orange-500 rounded-3xl"
                  style={{ borderColor: "FFB551" }}
                >
                  <div className="flex justify-between">
                    <div>
                      <img
                        className="h-20 border-2 border-orange-500 rounded-tl-3xl rounded-br-3xl"
                        style={{ borderColor: "FFB551" }}
                        src={cat}
                        alt="Cat Food"
                      />
                  ...
                  <Link to={`/SingleProductPage/${product._id}`}>
                    <img
                      ...
                      onClick={() =>
                        navigate(`/SingleProductPage/${product._id}`)
                      }
                    />
                  </Link>
                  <h2
                    className="text-center text-xl font-lilita"
                    style={{ color: "#2EC4B6" }}
                  >
                    <Link to={`/SingleProductPage/${product._id}`}>
                      {product.productName}
                  ...
                  <div className="border-none border-orange-500 rounded-b-3xl overflow-hidden pt-3">
                    <h2
                      className="text-white text-center text-xl font-lilita pt-3 pb-3"
                      style={{ backgroundColor: "#FFB551" }}
                    >
                      Price: ${product.price}
                    </h2>
                  </div>
